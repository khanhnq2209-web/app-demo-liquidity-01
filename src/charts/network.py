...
_TT_NETWORK = JsCode("function(p){if(p.dataType==='edge'){return p.data.source+' → '+p.data.target+'<br/>'+(p.data.value/1e9).toFixed(2)+' Tỷ VND';}return p.name;}")
